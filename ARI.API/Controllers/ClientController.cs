...
public class ClientController : ControllerBase
{
    ...
    public async Task Connect()
    {
        ...
        RegisterClientTool(codeThread, ws, "edit_file",
            "Edit a file by replacing one or more line ranges with new text. REQUIREMENT: read_file the file first — you edit BY LINE NUMBER using the 1-based line numbers shown by read_file/search_files. Set start_line and end_line (inclusive) and new_string (the replacement text only; an empty string deletes the range). To change several places at once, pass an 'edits' array of {start_line, end_line, new_string}. Use write_file for a new file or a full rewrite.",
            new { type = "object", properties = new { path = new { type = "string", description = "File path relative to project root" }, start_line = new { type = "integer", description = "First line to replace (1-based, inclusive, as shown by read_file/search_files)." }, end_line = new { type = "integer", description = "Last line to replace (1-based, inclusive). Defaults to start_line." }, new_string = new { type = "string", description = "Replacement text for the line range — the replacement code only. Empty string deletes the range." }, edits = new { type = "array", description = "Batch several changes; each item is {start_line, end_line, new_string}." } }, required = new[] { "path" } },
